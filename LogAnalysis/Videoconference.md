...
1) Total records present log?  
`cat meeting.json | wc -l` will give the number of entries.

2) Unique users recorded in log?  
...
7) Meeting ID with most participants?  
`cat meeting.json | cut -d':' -f6 | cut -d',' -f1 | sort | uniq -c | sort -n` This will print just the meeting id's, and then sort them, and then tell how many times each meeting is listed, which should be a count of how many users, this worked, but if it didnt, you could have the first cut include more, or use the jq command I learned later to count by users specifically.

8) Date with most calls?  
...
9) Date with highest average bytes processed?  
...
10) Unauthorized user, meeting ID?  
...
11) Account was hacked, name of person?  
